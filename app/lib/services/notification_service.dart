...
class NotificationService {
  ...
  Future<void> notifyAdminDeliveryCompleted({
    ...
    if (adminId != null) {
      await sendNotification(
        ...
        body:
            '$travelerName completed delivery of "$packageTitle" for ETB ${amount.toStringAsFixed(0)}. '
  ...
  Future<void> notifyTravelerDeliveryCancelledBySender({
    ...
    await sendNotification(
      ...
      body:
          'The sender cancelled "$packageTitle" before pickup. No action needed.',
  ...
  Future<void> checkAndReportExpiredDeliveries() async {
    // Same UTC caveat as DataService.expireOldRequests(): must convert to
    // UTC before formatting, or a bare local-time string gets misread as
    // UTC server-side and the cutoff ends up hours off from intended.
    ...
    final data = await _sb
        ...
        .inFilter('status', ['accepted', 'in_transit']).lt('pickup_at', cutoff);
    ...
    for (final d in overdue) {
      ...
      await sendNotification(
        ...
        body:
            'Your package "$pkg" has not been delivered within the expected time. '
      ...
      if (adminId != null) {
        await sendNotification(
          ...
          body:
              'Delivery of "$pkg" (id: ${deliveryId.substring(0, 8)}) has exceeded 4 hours. '
